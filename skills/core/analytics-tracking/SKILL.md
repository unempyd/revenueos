...
description: "Compatibility skill for analytics tracking. Use when a user or upstream workflow invokes this name; route the task to `analytics` while preserving the requested scope and source-specific intent."
license: Apache-2.0
metadata:
  version: "1.0.0"
  domain: "marketing"
...
Use this skill for **analytics tracking**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.

**Compatibility routing:** this source-facing name delegates to `analytics` while preserving its narrower semantics.
...
1. Read the task as `analytics-tracking` for compatibility, then apply the workflow in `analytics`.
