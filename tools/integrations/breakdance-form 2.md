...
WordPress visual builder form component for collecting leads and website inquiries.
...
```bash
wp plugin status breakdance-form
```
...
```bash
POST https://example.com/wp-json/breakdance-form/v1/records/{record_id}
...
```bash
wp option list --search='breakdance-form' --format=table
```
