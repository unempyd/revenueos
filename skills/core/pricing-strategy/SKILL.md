...
description: "Compatibility skill for pricing strategy. Use when a user or upstream workflow invokes this name; route the task to `pricing` while preserving the requested scope and source-specific intent."
license: Apache-2.0
metadata:
  version: "1.0.0"
  domain: "marketing"
...
Use this skill for **pricing strategy**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.

**Compatibility routing:** this source-facing name delegates to `pricing` while preserving its narrower semantics.
...
1. Read the task as `pricing-strategy` for compatibility, then apply the workflow in `pricing`.
