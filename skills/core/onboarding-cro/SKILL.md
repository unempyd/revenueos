---
name: onboarding-cro
description: "Compatibility skill for onboarding cro. Use when a user or upstream workflow invokes this name; route the task to `onboarding` while preserving the requested scope and source-specific intent."
license: Apache-2.0
metadata:
  version: "1.0.0"
  domain: "marketing"
...
Use this skill for **onboarding cro**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.

**Compatibility routing:** this source-facing name delegates to `onboarding` while preserving its narrower semantics.
...
1. Read the task as `onboarding-cro` for compatibility, then apply the workflow in `onboarding`.
