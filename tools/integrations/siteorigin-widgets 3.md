...
WordPress widget framework for building reusable content blocks and page sections.
...
```bash
wp plugin status siteorigin-widgets-bundle
```
...
```bash
POST https://example.com/wp-json/siteorigin-widgets-bundle/v1/records/{record_id}
...
```bash
wp option list --search='siteorigin-widgets-bundle' --format=table
```
