---
name: schema
description: "Design, validate and implement structured data that accurately represents visible page content and current search-engine eligibility rules."
license: Apache-2.0
metadata:
  version: "1.0.0"
  domain: "seo-geo"
...
Use this skill for **schema**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.
...
1. Define the exact `schema` objective, audience/scope, constraints and success metric before recommending action.
...
3. Collect the minimum evidence needed for schema. Distinguish direct observations from assumptions and proxies.
4. Execute the schema analysis or artifact using the domain checklist below; prefer specific outputs over generic best-practice lists.
