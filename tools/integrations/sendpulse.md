...
Marketing automation platform for email, SMS, chatbots, and transactional messaging.
...
- **Get token**: Developer settings, API settings, private app settings, or webhook settings inside the SendPulse dashboard
...
```bash
GET https://api.sendpulse.com/v1/records?limit=50
...
```bash
GET https://api.sendpulse.com/v1/records/{record_id}
...
```bash
POST https://api.sendpulse.com/v1/records
...
```bash
PATCH https://api.sendpulse.com/v1/records/{record_id}
...
```bash
POST https://api.sendpulse.com/v1/events
