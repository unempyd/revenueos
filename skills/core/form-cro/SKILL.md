---
name: form-cro
description: "Compatibility skill for form cro. Use when a user or upstream workflow invokes this name; route the task to `cro` while preserving the requested scope and source-specific intent."
license: Apache-2.0
metadata:
  version: "1.0.0"
  domain: "marketing"
...
Use this skill for **form cro**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.

**Compatibility routing:** this source-facing name delegates to `cro` while preserving its narrower semantics.
...
1. Read the task as `form-cro` for compatibility, then apply the workflow in `cro`.
