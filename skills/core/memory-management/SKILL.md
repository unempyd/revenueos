...
Use this skill for **memory management**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.
...
1. Define the exact `memory management` objective, audience/scope, constraints and success metric before recommending action.
...
3. Collect the minimum evidence needed for memory management. Distinguish direct observations from assumptions and proxies.
4. Execute the memory management analysis or artifact using the domain checklist below; prefer specific outputs over generic best-practice lists.
