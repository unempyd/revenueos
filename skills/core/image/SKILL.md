...
Use this skill for **image**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.
...
1. Define the exact `image` objective, audience/scope, constraints and success metric before recommending action.
...
3. Collect the minimum evidence needed for image. Distinguish direct observations from assumptions and proxies.
4. Execute the image analysis or artifact using the domain checklist below; prefer specific outputs over generic best-practice lists.
