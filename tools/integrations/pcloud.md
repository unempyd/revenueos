...
Cloud storage platform for files, folders, sharing, and backup workflows.
...
- **Get token**: Developer settings, API settings, private app settings, or webhook settings inside the pCloud dashboard
...
```bash
GET https://api.pcloud.com/v1/records?limit=50
...
```bash
GET https://api.pcloud.com/v1/records/{record_id}
...
```bash
POST https://api.pcloud.com/v1/records
...
```bash
PATCH https://api.pcloud.com/v1/records/{record_id}
...
```bash
POST https://api.pcloud.com/v1/events
