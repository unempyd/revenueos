...
WordPress subscription plugin for recurring payments, membership access, and billing workflows.
...
```bash
wp plugin status wpsubscription
```
...
```bash
POST https://example.com/wp-json/wpsubscription/v1/records/{record_id}
...
```bash
wp option list --search='wpsubscription' --format=table
```
