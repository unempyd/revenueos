...
| `REVENUEOS_LICENSE_SECRET` | panel (billing/licensing) | Only needed on the machine that *issues* licences (`revenueos license issue`) or verifies them against a signed key. Continuous operation (`revenueos orchestrator`'s tick loop, not `--once`) needs an installed Pro-or-higher licence; on Community tier the orchestrator container starts and exits immediately with `This feature needs RevenueOS Pro ($99/mo) or higher...` — that is expected product behaviour, not a crash. |
