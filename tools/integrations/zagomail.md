...
Email marketing and automation platform for subscribers, campaigns, and ecommerce messaging.
...
- **Get token**: Developer settings, API settings, private app settings, or webhook settings inside the Zagomail dashboard
...
```bash
GET https://api.zagomail.com/v1/records?limit=50
...
```bash
GET https://api.zagomail.com/v1/records/{record_id}
...
```bash
POST https://api.zagomail.com/v1/records
...
```bash
PATCH https://api.zagomail.com/v1/records/{record_id}
...
```bash
POST https://api.zagomail.com/v1/events
