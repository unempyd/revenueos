---
name: geo-prospect
description: "Compatibility skill for geo prospect. Use when a user or upstream workflow invokes this name; route the task to `prospecting` while preserving the requested scope and source-specific intent."
license: Apache-2.0
metadata:
  version: "1.0.0"
  domain: "seo-geo"
...
Use this skill for **geo prospect**. Start from the user’s concrete objective and available evidence; do not substitute generic marketing advice for task-specific analysis.

**Compatibility routing:** this source-facing name delegates to `prospecting` while preserving its narrower semantics.
...
1. Read the task as `geo-prospect` for compatibility, then apply the workflow in `prospecting`.
