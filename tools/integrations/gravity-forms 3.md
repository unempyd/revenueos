...
Advanced WordPress form builder for complex forms, payments, surveys, and lead capture.
...
```bash
wp plugin status gravity-forms
```
...
```bash
POST https://example.com/wp-json/gravity-forms/v1/records/{record_id}
...
```bash
wp option list --search='gravity-forms' --format=table
```
